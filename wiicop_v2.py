...
class plot_cop:
    ...
    def __init__(self,aqc_info,BB_X,BB_Y):
        ...
        self.fig.canvas.set_window_title(acqobj.aqc_name())
        ...
        # create text box
        self.text_h = ax.text(0.02, 0.98, text_start, verticalalignment='top',horizontalalignment='left',
        transform=ax.transAxes, fontsize=12, bbox=dict(facecolor='white'), gid = 'notrec')
        # create timer object
        if self.acq_info['acq_time'] != 'inf':
            acq_timer = self.fig.canvas.new_timer(interval=acq_time_ms)
...
# read data from the balance board
